...
## velocimetro
def obtener_velocimetro_s13_suple_gestante(
    anio: str,
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.callproc('fn_obtener_velocimetro_s13_suple_gestante', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
def obtener_grafico_mensual_s13_suple_gestante(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_grafico_mensualizado_s13_suple_gestante', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
## grafico variables
def obtener_variables_s13_suple_gestante(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_obtener_variables_s13_suple_gestante', [
                anio,
                mes_inicio,
                mes_fin,
                red,
            ...
            if row and len(row) >= 15:
                return [{
                    ...
                    'avance_suple1': float(row[2]) if row[2] is not None else 0.0,
                    'num_suple2': int(row[3]) if row[3] is not None else 0,
                    'avance_suple2': float(row[4]) if row[4] is not None else 0.0,
                    'num_suple3': int(row[5]) if row[5] is not None else 0,
                    'avance_suple3': float(row[6]) if row[6] is not None else 0.0,
                    'num_suple4': int(row[7]) if row[7] is not None else 0,
                    'avance_suple4': float(row[8]) if row[8] is not None else 0.0,
                    'num_suple5': int(row[9]) if row[9] is not None else 0,
                    'avance_suple5': float(row[10]) if row[10] is not None else 0.0,
                    'num_hb2': int(row[11]) if row[11] is not None else 0,
                    'avance_hb2': float(row[12]) if row[12] is not None else 0.0,
                    'num_hb3': int(row[13]) if row[13] is not None else 0,
                    'avance_hb3': float(row[14]) if row[14] is not None else 0.0
                }]
            else:
                # Sin datos en la tabla o número incorrecto de columnas
                if row:
...
def obtener_variables_detallado_s13_suple_gestante(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_obtener_variables_detallado_s13_suple_gestante', [
                anio,
                mes_inicio,
                mes_fin,
                red,
                ...
                for row in rows:
                    if len(row) >= 18:
                        resultados.append({
                            ...
                            'd_num_suple1': int(row[12]) if row[12] is not None else 0,
                            'd_avance_suple1': float(row[13]) if row[13] is not None else 0.0,
                            'd_num_suple2': int(row[14]) if row[14] is not None else 0,
                            'd_avance_suple2': float(row[15]) if row[15] is not None else 0.0,
                            'd_num_suple3': int(row[16]) if row[16] is not None else 0,
                            'd_avance_suple3': float(row[17]) if row[17] is not None else 0.0,
                            'd_num_suple4': int(row[18]) if row[18] is not None else 0,
                            'd_avance_suple4': float(row[19]) if row[19] is not None else 0.0,
                            'd_num_suple5': int(row[20]) if row[20] is not None else 0,
                            'd_avance_suple5': float(row[21]) if row[21] is not None else 0.0,
...
def obtener_grafico_por_redes_s13_suple_gestante(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
...
def obtener_grafico_por_microredes_s13_suple_gestante(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_grafico_microredes_s13_suple_gestante', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
def obtener_grafico_por_establecimientos_s13_suple_gestante(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_grafico_establecimientos_s13_suple_gestante', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
def obtener_seguimiento_s13_suple_gestante(
    anio=None,
    mes_inicio=None,
    mes_fin=None,
    provincia='',
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                """
                SELECT * FROM fn_seg_suple_gestante(
                    %s, %s, %s, %s, %s, %s, %s, %s, %s
                )
                """,
                [

...
from .queries import obtener_velocimetro_s22_sin_anemia_nino, obtener_grafico_mensual_s22_sin_anemia_nino, obtener_variables_s22_sin_anemia_nino, obtener_variables_detallado_s22_sin_anemia_nino, obtener_grafico_por_redes_s22_sin_anemia_nino
from .queries import obtener_grafico_por_microredes_s22_sin_anemia_nino, obtener_grafico_por_establecimientos_s22_sin_anemia_nino
from .queries import obtener_seguimiento_s22_sin_anemia_nino
...
######################################
## VELOCIMETRO
def process_velocimetro_s22_sin_anemia_nino(resultados_velocimetro_s22_sin_anemia_nino: List[Dict]) -> Dict[str, List]:
    """
    ...
    """
    # Validar entrada
    if not resultados_velocimetro_s22_sin_anemia_nino or len(resultados_velocimetro_s22_sin_anemia_nino) == 0:
    ...
    row = resultados_velocimetro_s22_sin_anemia_nino[0]
...
def obtener_resumen_indicador_s22_sin_anemia_nino(anio, mes_inicio, mes_fin, red, microred, establecimiento, provincia=None, distrito=None):
    """
    Obtiene un resumen detallado del indicador 
    """
    datos_base = obtener_velocimetro_s22_sin_anemia_nino(anio, mes_inicio, mes_fin, red, microred, establecimiento, provincia, distrito)
...
def process_avance_mensual_s22_sin_anemia_nino(resultados_avance_mensual_s22_sin_anemia_nino: List[Dict]) -> Dict[str, List]:
    ...
    for index, row in enumerate(resultados_avance_mensual_s22_sin_anemia_nino):
        try:
...
## GRAFICO VARIABLES 
def process_variables_s22_sin_anemia_nino(resultados_variables_s22_sin_anemia_nino: List[Dict]) -> Dict[str, List]:
    ...
    for index, row in enumerate(resultados_variables_s22_sin_anemia_nino):
        try:
            # Verifica que el diccionario tenga las claves necesarias
            required_keys = {'den_variable','num_hierro30d','avance_hierro30d','num_hierro4m','avance_hierro4m','num_hb30d','avance_hb30d','num_hb4m','avance_hb4m','num_hb6m','avance_hb6m'}
            ...
            num_hierro30d = row['num_hierro30d']
            avance_hierro30d = row['avance_hierro30d']
            num_hierro4m = row['num_hierro4m']
            avance_hierro4m = row['avance_hierro4m']
            num_hb30d = row['num_hb30d']
            avance_hb30d = row['avance_hb30d']
            num_hb4m = row['num_hb4m']
            avance_hb4m = row['avance_hb4m']
            num_hb6m = row['num_hb6m']
            avance_hb6m = row['avance_hb6m']
            
            # Agrega los valores a la lista
            data['den_variable'].append(den_variable)
            data['num_hierro30d'].append(num_hierro30d)
            data['avance_hierro30d'].append(avance_hierro30d)
            data['num_hierro4m'].append(num_hierro4m)
            data['avance_hierro4m'].append(avance_hierro4m)
            data['num_hb30d'].append(num_hb30d)
            data['avance_hb30d'].append(avance_hb30d)
            data['num_hb4m'].append(num_hb4m)
            data['avance_hb4m'].append(avance_hb4m)
            data['num_hb6m'].append(num_hb6m)
            data['avance_hb6m'].append(avance_hb6m)
...
def process_variables_detallado_s22_sin_anemia_nino(resultados_variables_detallado_s22_sin_anemia_nino: List[Dict]) -> Dict[str, List]:
    ...
    for index, row in enumerate(resultados_variables_detallado_s22_sin_anemia_nino):
        try:
            # Verifica que el diccionario tenga las claves necesarias
            ...
            'd_num_hierro30d','d_avance_hierro30d','d_num_hierro4m','d_avance_hierro4m','d_num_hb30d','d_avance_hb30d','d_num_hb4m','d_avance_hb4m','d_num_hb6m','d_avance_hb6m'}
            ...
            d_num_hierro30d = row['d_num_hierro30d']
            d_avance_hierro30d = row['d_avance_hierro30d']
            d_num_hierro4m = row['d_num_hierro4m']
            d_avance_hierro4m = row['d_avance_hierro4m']
            d_num_hb30d = row['d_num_hb30d']
            d_avance_hb30d = row['d_avance_hb30d']
            d_num_hb4m = row['d_num_hb4m']
            d_avance_hb4m = row['d_avance_hb4m']
            d_num_hb6m = row['d_num_hb6m']
            d_avance_hb6m = row['d_avance_hb6m']
            ...
            data['d_num_hierro30d'].append(d_num_hierro30d)
            data['d_avance_hierro30d'].append(d_avance_hierro30d)
            data['d_num_hierro4m'].append(d_num_hierro4m)
            data['d_avance_hierro4m'].append(d_avance_hierro4m)
            data['d_num_hb30d'].append(d_num_hb30d)
            data['d_avance_hb30d'].append(d_avance_hb30d)
            data['d_num_hb4m'].append(d_num_hb4m)
            data['d_avance_hb4m'].append(d_avance_hb4m)
            data['d_num_hb6m'].append(d_num_hb6m)
            data['d_avance_hb6m'].append(d_avance_hb6m)
...
def process_grafico_por_redes_s22_sin_anemia_nino(resultados_grafico_por_redes_s22_sin_anemia_nino: List[Dict]) -> Dict[str, List]:
    ...
    for index, row in enumerate(resultados_grafico_por_redes_s22_sin_anemia_nino):
        try:
            # Verifica que el diccionario tenga las claves necesarias
...
def process_grafico_por_microredes_s22_sin_anemia_nino(resultados_grafico_por_microredes_s22_sin_anemia_nino: List[Dict]) -> Dict[str, List]:
    ...
    for index, row in enumerate(resultados_grafico_por_microredes_s22_sin_anemia_nino):
        try:
            # Verifica que el diccionario tenga las claves necesarias
...
def process_grafico_por_establecimientos_s22_sin_anemia_nino(resultados_grafico_por_establecimientos_s22_sin_anemia_nino: List[Dict]) -> Dict[str, List]:
    ...
    for index, row in enumerate(resultados_grafico_por_establecimientos_s22_sin_anemia_nino):
        try:
            # Verifica que el diccionario tenga las claves necesarias
...
def index_s22_sin_anemia_nino(request):
    """
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest':
        try:
            ...
            resultados_velocimetro_s22_sin_anemia_nino = obtener_velocimetro_s22_sin_anemia_nino(
                anio=anio,
            ...
            resumen_s22_sin_anemia_nino = obtener_resumen_indicador_s22_sin_anemia_nino(
                anio=anio,
                mes_inicio=mes_seleccionado_inicio,
                mes_fin=mes_seleccionado_fin,
                red=red_seleccionada,
            ...
            resultados_grafico_mensual_s22_sin_anemia_nino = obtener_grafico_mensual_s22_sin_anemia_nino(
                anio=anio,
            ...
            resultados_variables_s22_sin_anemia_nino = obtener_variables_s22_sin_anemia_nino(
                anio=anio,
                mes_inicio=mes_seleccionado_inicio,
            ...
            resultados_variables_detallado_s22_sin_anemia_nino = obtener_variables_detallado_s22_sin_anemia_nino(
                anio=anio,
            ...
            resultados_grafico_por_redes_s22_sin_anemia_nino = obtener_grafico_por_redes_s22_sin_anemia_nino(
                anio=anio,
                mes_inicio=mes_seleccionado_inicio,  # Usa el mes de inicio seleccionado
                mes_fin=mes_seleccionado_fin,        # Usa el mes de fin seleccionado
                red=red_seleccionada,
            ...
            resultados_grafico_por_microredes_s22_sin_anemia_nino = obtener_grafico_por_microredes_s22_sin_anemia_nino(
                anio=anio,
                mes_inicio=mes_seleccionado_inicio,  # Usa el mes de inicio seleccionado
                mes_fin=mes_seleccionado_fin,        # Usa el mes de fin seleccionado
                red=red_seleccionada,
            ...
            resultados_grafico_por_establecimientos_s22_sin_anemia_nino = obtener_grafico_por_establecimientos_s22_sin_anemia_nino(
                anio=anio,
                mes_inicio=mes_seleccionado_inicio,  # Usa el mes de inicio seleccionado
                mes_fin=mes_seleccionado_fin,        # Usa el mes de fin seleccionado
                red=red_seleccionada,
            ...
            data = {
               **process_velocimetro_s22_sin_anemia_nino(resultados_velocimetro_s22_sin_anemia_nino),
               **process_avance_mensual_s22_sin_anemia_nino(resultados_grafico_mensual_s22_sin_anemia_nino),
               **process_variables_s22_sin_anemia_nino(resultados_variables_s22_sin_anemia_nino),
               **process_variables_detallado_s22_sin_anemia_nino(resultados_variables_detallado_s22_sin_anemia_nino),
               **process_grafico_por_redes_s22_sin_anemia_nino(resultados_grafico_por_redes_s22_sin_anemia_nino),
               **process_grafico_por_microredes_s22_sin_anemia_nino(resultados_grafico_por_microredes_s22_sin_anemia_nino),
               **process_grafico_por_establecimientos_s22_sin_anemia_nino(resultados_grafico_por_establecimientos_s22_sin_anemia_nino)
            }

            # Agregar datos del resumen a la respuesta
            if resumen_s22_sin_anemia_nino:
                data['r_numerador_resumen'] = resumen_s22_sin_anemia_nino['numerador']
                data['r_denominador_resumen'] = resumen_s22_sin_anemia_nino['denominador']
                data['r_avance_resumen'] = resumen_s22_sin_anemia_nino['avance']
                data['r_brecha'] = resumen_s22_sin_anemia_nino['brecha']
                data['r_clasificacion'] = resumen_s22_sin_anemia_nino['clasificacion']
                data['r_color'] = resumen_s22_sin_anemia_nino['color']
                data['r_icono'] = resumen_s22_sin_anemia_nino['icono']
    ...
    return render(request, 's22_sin_anemia_nino/index_s22_sin_anemia_nino.html', context)
...
def get_establecimientos_s22_sin_anemia_nino_h(request, establecimiento_id):
    """
    ...
    return render(request, 's22_sin_anemia_nino/establecimientos_h.html', context)


def p_microredes_establec_s22_sin_anemia_nino_h(request):
    """
    ...
    return render(request, 's22_sin_anemia_nino/partials/p_microredes_establec_h.html', context)


def p_establecimientos_s22_sin_anemia_nino_h(request):
    """
    ...
    return render(request, 's22_sin_anemia_nino/partials/p_establecimientos_h.html', context)


def p_distritos_s22_sin_anemia_nino_h(request):
    """
    ...
    return render(request, 's22_sin_anemia_nino/partials/p_distritos.html', context)
...
def get_redes_s22_sin_anemia_nino(request, redes_id):
    """
    Renderiza el formulario de reportes por REDES.
    """
    context = _get_context_base_con_filtros(include_meses=True)
    return render(request, 's22_sin_anemia_nino/components/salud/redes.html', context)


def get_microredes_s22_sin_anemia_nino(request, microredes_id):
    """
    ...
    return render(request, 's22_sin_anemia_nino/components/salud/microredes.html', context)


def get_establecimientos_s22_sin_anemia_nino(request, establecimiento_id):
    ...
    return render(request, 's22_sin_anemia_nino/components/salud/establecimientos.html', context)
...
def p_microredes_s22_sin_anemia_nino(request):
    """
    ...
    return render(request, 's22_sin_anemia_nino/partials/p_microredes.html', context)


def p_microredes_establec_s22_sin_anemia_nino(request):
    """
    ...
    return render(request, 's22_sin_anemia_nino/partials/p_microredes_establec.html', {
        'microredes': microredes,
        'red': red,
    })
...
# ============================================
def p_establecimientos_s22_sin_anemia_nino(request):
    """
    ...
    return render(request, 's22_sin_anemia_nino/partials/p_establecimientos.html', {
...
def get_provincias_s22_sin_anemia_nino(request, provincia_id):
    provincias = (
    ...
    return render(request, 's22_sin_anemia_nino/components/municipio/provincias.html', context)

## SEGUIMIENTO POR DISTRITOS
def get_distritos_s22_sin_anemia_nino(request, distrito_id):
    provincias = (
                MAESTRO_HIS_ESTABLECIMIENTO
                .objects.filter(Descripcion_Sector='GOBIERNO REGIONAL')
                .annotate(ubigueo_filtrado=Substr('Ubigueo_Establecimiento', 1, 4))
    ...
    return render(request, 's22_sin_anemia_nino/components/municipio/distritos.html', context)


def p_distrito_s22_sin_anemia_nino(request):
...
COLUMN_WIDTHS = {
    ...
    'I': 7, 'J': 9, 'K': 5, 'L': 4, 'M':4, 'N': 4, 'O': 4, 'P': 4,'Q': 9,
    'R': 10,'S': 5, 'T': 10, 'U':5, 'V':10, 'W':8, 'X':5, 'Y':10, 'Z':8,'AA':5, 
    'AB':10,'AC':8,'AD':5,'AE':10,'AF':9,'AG':10,'AH':20,'AI':10,'AJ':20,'AK':10,'AL':25
}
...
HEADERS_CONFIG = [
    ...
    ('Z9', 'VAL','celeste'),
    ('AA9', '4M%','celeste'),
    ('AB9', 'HB 6M','morado_claro'),
    ('AC9', 'VAL','morado_claro'),
    ('AD9', '6M%','morado_claro'),
    ('AE9', 'IND','blue'),
    ('AF9', 'MES','blue'),
    ('AG9', 'COD RED','orange'),
    ('AH9', 'RED','orange'),
    ('AI9', 'COD MICRO','orange'),
    ('AJ9', 'MICRORED','orange'),
    ('AK9', 'COD EESS','orange'),
    ('AL9', 'ESTABLECIMIENTO', 'orange')
]
...
MERGE_CELLS_CONFIG = [
    ...
    ('B6', 'Q6'), ('R6', 'U6'),('V6','AD6'), 
    # Fila 7
    ('B7','E7'),('F7','P7'),('R7','S7'),('T7','U7'),('V7','X7'),('Y7','AA7'),('AB7','AD7'),
    # Fila 8
    ('B8','E8'),('F8','K8'),('L8','P8'),('R8','S8'),('T8','U8'),('V8','X8'),('Y8','AA8'),('AB8','AD8'),
    # Columnas 
    ('Q7','Q8'),('AE6','AF8')
]
...
class RptSinAnemiaNino(BaseExcelReportView):
    """Reporte de captación de gestantes."""
    
    filename = "rpt_s22_sin_anemia_nino.xlsx"
    ...
    def get_data(self, params):
        return obtener_seguimiento_s22_sin_anemia_nino(
...
class RptSinAnemiaNinoMicroRed(BaseExcelReportView):
    """Reporte de población por microred."""
    
    filename = "rpt_s22_sin_anemia_nino_microred.xlsx"
    sheet_name = "Seguimiento"
...
class RptSinAnemiaNinoEstablec(BaseExcelReportView):
    """Reporte de población por establecimiento."""
    
    filename = "rpt_s22_sin_anemia_nino_establecimiento.xlsx"
    sheet_name = "Seguimiento"
...
def _style_header_sections(ws, style_mgr):
    ...
    sections_config = {
        ...
        'R5': ('N° de niñas y niños del denominador que reciben suplementación preventiva con hierro y dosajes de hemoglobina', 'naranja_claro', 10, True),
        # Fila 6: B6:M6, N6:S6, T6:Y6, Z6:AA6 son rangos combinados
        'B6': ('N° de niñas y niños de 6 meses de edad con prematuridad leve y/o bajo peso al nacer,registrados en el padrón nominal (con DNI o CNV en línea), y sin anemia', 'gray', 10, True),
        'R6': ('Entrega de Hierro', 'gray', 10, True),
        'V6': ('Entrega de Dosaje de Hemoglobina', 'gray', 10, True),
        ...
        'B7': ('Padrón nominal de niños y niñas menores de 06 años', 'plomo_claro', 7, True),
        'F7': ('Se excluye a las niñas y niños prematuros (menor de 37 semanas de gestación) y con bajo peso al nacer (menor a 2500 gr), a partir del CNV en línea. ', 'plomo_claro', 7, False),
        'R7': ('Reciben un frasco de hierro a los 30 dias de edad', 'plomo_claro', 7, False),
        'T7': ('Reciben un frasco de hierro entre el tercer o cuarto mes de edad', 'plomo_claro', 7, False),
        'V7': ('Cuentan con dosaje de hemoglobina a los 30 dias de edad', 'plomo_claro', 7, False),
        'Y7': ('Cuentan con dosaje de hemoglobina a los 3 meses de iniciada la  suplementación', 'plomo_claro', 7, False),
        'AB7': ('Cuentan con dosaje de hemoglobina a los 6 meses de edad', 'plomo_claro', 7, False),
        ...
        'F8': ('DX = D509 ó D649 + TD= D', 'azul_claro', 7, False),
        'L8': ('VALIDADORES', 'azul_claro', 7, False),      
        'R8': ('DX = 99199.17', 'azul_claro', 7, False),
        'T8': ('DX = 99199.17', 'azul_claro', 7, False),
        'V8': ('DX = 85018 ó 85018.01 ó 85031', 'azul_claro', 7, False),
        'Y8': ('DX = 85018 ó 85018.01 ó 85031', 'azul_claro', 7, False),
        'AB8': ('DX = 85018 ó 85018.01 ó 85031', 'azul_claro', 7, False),
...
def _add_titles(ws, style_mgr):
    ...
    titles = [
        ...
        ('B4', 'SEGUIMIENTO NOMINAL: SI-02.02: PORCENTAJE DE NIÑAS Y NIÑOS DE SEIS (06) MESES DE EDAD, PREMATUROS Y/O BAJO PESO AL NACER SIN ANEMIA; QUE RECIBEN SUPLEMENTACIÓN PREVENTIVA CON HIERRO Y DOSAJES DE HEMOGLOBINA', 12, True, '000000'),
    ]
...
def _write_data(ws, results, style_mgr):
    ...
    left_align_cols = {8, 36, 38}
    # Columnas con check/x marks
    check_cols = {12,13,14,15,16,19,21,24,27,30}
    ...
    for row_idx, record in enumerate(results, start=10):
        for col_idx, value in enumerate(record.values(), start=2):
            ...
            if col_idx == 31:  # Columna INDICADOR

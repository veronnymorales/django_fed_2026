...
DEFAULT_VARIABLES_DETALLADO_DATA = {
    ...
    'd_num_variable': 0,
    'd_num_hierro30d': 0,
    'd_avance_hierro30d': 0.0,
    'd_num_hierro4m': 0,
    'd_avance_hierro4m': 0.0,
    'd_num_hb30d': 0,
    'd_avance_hb30d': 0.0,
    'd_num_hb4m': 0,
    'd_avance_hb4m': 0.0,
...
## velocimetro
def obtener_velocimetro_s22_sin_anemia_nino(
    anio: str,
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.callproc('fn_obtener_velocimetro_s22_sin_anemia_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
def obtener_grafico_mensual_s22_sin_anemia_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_grafico_mensualizado_s22_sin_anemia_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
## grafico variables
def obtener_variables_s22_sin_anemia_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_obtener_variables_s22_sin_anemia_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
            ...
            if row and len(row) >= 11:
                return [{
                    ...
                    'num_hierro30d': int(row[1]) if row[1] is not None else 0,
                    'avance_hierro30d': float(row[2]) if row[2] is not None else 0.0,
                    'num_hierro4m': int(row[3]) if row[3] is not None else 0,
                    'avance_hierro4m': float(row[4]) if row[4] is not None else 0.0,
                    'num_hb30d': int(row[5]) if row[5] is not None else 0,
                    'avance_hb30d': float(row[6]) if row[6] is not None else 0.0,
                    'num_hb4m': int(row[7]) if row[7] is not None else 0,
                    'avance_hb4m': float(row[8]) if row[8] is not None else 0.0,
                    'num_hb6m': int(row[9]) if row[9] is not None else 0,
                    'avance_hb6m': float(row[10]) if row[10] is not None else 0.0
                }]
            else:
                # Sin datos en la tabla o número incorrecto de columnas
                if row:
...
def obtener_variables_detallado_s22_sin_anemia_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_obtener_variables_detallado_s22_sin_anemia_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
                ...
                for row in rows:
                    if len(row) >= 22:
                        resultados.append({
                            ...
                            'd_num_hierro30d': int(row[12]) if row[12] is not None else 0,
                            'd_avance_hierro30d': float(row[13]) if row[13] is not None else 0.0,
                            'd_num_hierro4m': int(row[14]) if row[14] is not None else 0,
                            'd_avance_hierro4m': float(row[15]) if row[15] is not None else 0.0,
                            'd_num_hb30d': int(row[16]) if row[16] is not None else 0,
                            'd_avance_hb30d': float(row[17]) if row[17] is not None else 0.0,
                            'd_num_hb4m': int(row[18]) if row[18] is not None else 0,
                            'd_avance_hb4m': float(row[19]) if row[19] is not None else 0.0,
                            'd_num_hb6m': int(row[20]) if row[20] is not None else 0,
                            'd_avance_hb6m': float(row[21]) if row[21] is not None else 0.0
                        })
                    else:
...
def obtener_grafico_por_redes_s22_sin_anemia_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_grafico_redes_s22_sin_anemia_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
def obtener_grafico_por_microredes_s22_sin_anemia_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_grafico_microredes_s22_sin_anemia_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
def obtener_grafico_por_establecimientos_s22_sin_anemia_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_grafico_establecimientos_s22_sin_anemia_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
def obtener_seguimiento_s22_sin_anemia_nino(
    anio=None,
    mes_inicio=None,
    mes_fin=None,
    provincia='',
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                """
                SELECT * FROM fn_seg_sin_anemia_nino(
                    %s, %s, %s, %s, %s, %s, %s, %s, %s
                )
                """,
                [

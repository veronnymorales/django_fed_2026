...
DEFAULT_VARIABLES_DATA = {
    'den_variable': 0,
    'num_vac_bcg': 0,
    'avance_num_vac_bcg': 0.0,
    'num_vac_hvb': 0,
    'avance_num_vac_hvb': 0.0,
    'num_vac': 0,
    'avance_num_vac': 0.0,
    'num_cred_rn1': 0,
    'avance_num_cred_rn1': 0.0,
    'num_cred_rn2': 0,
    'avance_num_cred_rn2': 0.0,
    'num_cred_rn3': 0,
    'avance_num_cred_rn3': 0.0,
    'num_tamizaje_neo': 0,
    'avance_num_tamizaje_neo': 0.0
}
...
DEFAULT_VARIABLES_DETALLADO_DATA = {
    ...
    'd_num_variable':0,
    'd_num_vac_bcg': 0,
    'd_avance_num_vac_bcg': 0.0,
    'd_num_vac_hvb': 0,
    'd_avance_num_vac_hvb': 0.0,
    'd_num_vac': 0,
    'd_avance_num_vac': 0.0,
    'd_num_cred_rn1': 0,
    'd_avance_num_cred_rn1': 0.0,
    'd_num_cred_rn2': 0,
    'd_avance_num_cred_rn2': 0.0,
    'd_num_cred_rn3': 0,
    'd_avance_num_cred_rn3': 0.0,
    'd_num_tamizaje_neo': 0,
    'd_avance_num_tamizaje_neo': 0.0
}
...
## velocimetro
def obtener_velocimetro_mc31_paquete_neonato(
    anio: str,
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.callproc('fn_obtener_velocimetro_mc31_paquete_neonato', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
def obtener_grafico_mensual_mc31_paquete_neonato(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_grafico_mensualizado_mc31_paquete_neonato', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
## grafico variables
def obtener_variables_mc31_paquete_neonato(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_obtener_variables_mc31_paquete_neonato', [
                anio,
                mes_inicio,
                mes_fin,
                red,
            ...
            if row and len(row) >= 15:
                return [{
                    ...
                    'num_vac_bcg': int(row[1]) if row[1] is not None else 0,
                    'avance_num_vac_bcg': float(row[2]) if row[2] is not None else 0.0,
                    'num_vac_hvb': int(row[3]) if row[3] is not None else 0,
                    'avance_num_vac_hvb': float(row[4]) if row[4] is not None else 0.0,
                    'num_vac': int(row[5]) if row[5] is not None else 0,
                    'avance_num_vac': float(row[6]) if row[6] is not None else 0.0,
                    'num_cred_rn1': int(row[7]) if row[7] is not None else 0,
                    'avance_num_cred_rn1': float(row[8]) if row[8] is not None else 0.0,
                    'num_cred_rn2': int(row[9]) if row[9] is not None else 0,
                    'avance_num_cred_rn2': float(row[10]) if row[10] is not None else 0.0,
                    'num_cred_rn3': int(row[11]) if row[11] is not None else 0,
                    'avance_num_cred_rn3': float(row[12]) if row[12] is not None else 0.0,
                    'num_tamizaje_neo': int(row[13]) if row[13] is not None else 0,
                    'avance_num_tamizaje_neo': float(row[14]) if row[14] is not None else 0.0
                }]
            else:
                # Sin datos en la tabla o número incorrecto de columnas
                if row:
...
def obtener_variables_detallado_mc31_paquete_neonato(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_obtener_variables_detallado_mc31_paquete_neonato', [
                anio,
                mes_inicio,
                mes_fin,
                red,
                ...
                for row in rows:
                    if len(row) >= 26:
                        resultados.append({
                            ...
                            'd_num_vac_bcg': int(row[12]) if row[12] is not None else 0,
                            'd_avance_num_vac_bcg': float(row[13]) if row[13] is not None else 0.0,
                            'd_num_vac_hvb': int(row[14]) if row[14] is not None else 0,
                            'd_avance_num_vac_hvb': float(row[15]) if row[15] is not None else 0.0,
                            'd_num_vac': int(row[16]) if row[16] is not None else 0,
                            'd_avance_num_vac': float(row[17]) if row[17] is not None else 0.0,
                            'd_num_cred_rn1': int(row[18]) if row[18] is not None else 0,
                            'd_avance_num_cred_rn1': float(row[19]) if row[19] is not None else 0.0,
                            'd_num_cred_rn2': int(row[20]) if row[20] is not None else 0,
                            'd_avance_num_cred_rn2': float(row[21]) if row[21] is not None else 0.0,
                            'd_num_cred_rn3': int(row[22]) if row[22] is not None else 0,
                            'd_avance_num_cred_rn3': float(row[23]) if row[23] is not None else 0.0,
                            'd_num_tamizaje_neo': int(row[24]) if row[24] is not None else 0,
                            'd_avance_num_tamizaje_neo': float(row[25]) if row[25] is not None else 0.0
                        })
                    else:
                        logger.warning(f"Una fila retornó {len(row)} columnas en lugar de 26, omitiendo...")
...
def obtener_grafico_por_redes_mc31_paquete_neonato(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_grafico_redes_mc31_paquete_neonato', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
def obtener_grafico_por_microredes_mc31_paquete_neonato(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_grafico_microredes_mc31_paquete_neonato', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
def obtener_grafico_por_establecimientos_mc31_paquete_neonato(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_grafico_establecimientos_mc31_paquete_neonato', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
def obtener_seguimiento_mc31_paquete_neonato(
    anio=None,
    mes_inicio=None,
    mes_fin=None,
    provincia='',
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                """
                SELECT * FROM fn_seg_mc31_paquete_neonato(
                    %s, %s, %s, %s, %s, %s, %s, %s, %s
                )
                """,
                [

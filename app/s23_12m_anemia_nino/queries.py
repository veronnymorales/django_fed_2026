...
DEFAULT_VARIABLES_DATA = {
    'den_variable': 0,
    'num_hierro_trat1': 0,
    'avance_hierro_trat1': 0.0,
    'num_hierro_trat2': 0,
    'avance_hierro_trat2': 0.0,
    'num_hierro_trat3': 0,
    'avance_hierro_trat3': 0.0,
    'num_hb7m': 0,
    'avance_hb7m': 0.0,
    'num_hb8m': 0,
    'avance_hb8m': 0.0,
...
DEFAULT_VARIABLES_DETALLADO_DATA = {
    ...
    'd_den_variable': 0,
    'd_num_variable': 0,
    'd_num_hierro_trat1': 0,
    'd_avance_hierro_trat1': 0.0,
    'd_num_hierro_trat2': 0,
    'd_avance_hierro_trat2': 0.0,
    'd_num_hierro_trat3': 0,
    'd_avance_hierro_trat3': 0.0,
    'd_num_hb7m': 0,
    'd_avance_hb7m': 0.0,
    'd_num_hb8m': 0,
    'd_avance_hb8m': 0.0,
...
## velocimetro
def obtener_velocimetro_s23_12m_anemia_nino(
    anio: str,
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.callproc('fn_obtener_velocimetro_s23_12m_anemia_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
def obtener_grafico_mensual_s23_12m_anemia_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_grafico_mensualizado_s23_12m_anemia_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
## grafico variables
def obtener_variables_s23_12m_anemia_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_obtener_variables_s23_12m_anemia_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
            ...
            if row and len(row) >= 15:
                return [{
                    ...
                    'num_hierro_trat1': int(row[1]) if row[1] is not None else 0,
                    'avance_hierro_trat1': float(row[2]) if row[2] is not None else 0.0,
                    'num_hierro_trat2': int(row[3]) if row[3] is not None else 0,
                    'avance_hierro_trat2': float(row[4]) if row[4] is not None else 0.0,
                    'num_hierro_trat3': int(row[5]) if row[5] is not None else 0,
                    'avance_hierro_trat3': float(row[6]) if row[6] is not None else 0.0,
                    'num_hb7m': int(row[7]) if row[7] is not None else 0,
                    'avance_hb7m': float(row[8]) if row[8] is not None else 0.0,
                    'num_hb8m': int(row[9]) if row[9] is not None else 0,
                    'avance_hb8m': float(row[10]) if row[10] is not None else 0.0,
                    'num_hb9m': int(row[11]) if row[11] is not None else 0,
                    'avance_hb9m': float(row[12]) if row[12] is not None else 0.0,
                    'num_hb12m': int(row[13]) if row[13] is not None else 0,
                    'avance_hb12m': float(row[14]) if row[14] is not None else 0.0
                }]
            else:
                # Sin datos en la tabla o número incorrecto de columnas
                if row:
                    logger.warning(f"La consulta de variables retornó {len(row)} columnas en lugar de 5")
                else:
...
def obtener_variables_detallado_s23_12m_anemia_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_obtener_variables_detallado_s23_12m_anemia_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
                ...
                for row in rows:
                    if len(row) >= 26:
                        resultados.append({
                            ...
                            'd_num_hierro_trat1': int(row[12]) if row[12] is not None else 0,
                            'd_avance_hierro_trat1': float(row[13]) if row[13] is not None else 0.0,
                            'd_num_hierro_trat2': int(row[14]) if row[14] is not None else 0,
                            'd_avance_hierro_trat2': float(row[15]) if row[15] is not None else 0.0,
                            'd_num_hierro_trat3': int(row[16]) if row[16] is not None else 0,
                            'd_avance_hierro_trat3': float(row[17]) if row[17] is not None else 0.0,
                            'd_num_hb7m': int(row[18]) if row[18] is not None else 0,
                            'd_avance_hb7m': float(row[19]) if row[19] is not None else 0.0,
                            'd_num_hb8m': int(row[20]) if row[20] is not None else 0,
                            'd_avance_hb8m': float(row[21]) if row[21] is not None else 0.0,
                            'd_num_hb9m': int(row[22]) if row[22] is not None else 0,
                            'd_avance_hb9m': float(row[23]) if row[23] is not None else 0.0,
                            'd_num_hb12m': int(row[24]) if row[24] is not None else 0,
                            'd_avance_hb12m': float(row[25]) if row[25] is not None else 0.0
                        })
                    else:
                        logger.warning(f"Una fila retornó {len(row)} columnas en lugar de 16, omitiendo...")
...
def obtener_grafico_por_redes_s23_12m_anemia_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_grafico_redes_s23_12m_anemia_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
def obtener_grafico_por_microredes_s23_12m_anemia_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_grafico_microredes_s23_12m_anemia_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
def obtener_grafico_por_establecimientos_s23_12m_anemia_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_grafico_establecimientos_s23_12m_anemia_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
def obtener_seguimiento_s23_12m_anemia_nino(
    anio=None,
    mes_inicio=None,
    mes_fin=None,
    provincia='',
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                """
                SELECT * FROM fn_seg_12m_anemia_nino(
                    %s, %s, %s, %s, %s, %s, %s, %s, %s
                )
                """,
                [

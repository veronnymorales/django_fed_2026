...
DEFAULT_VARIABLES_DATA = {
    'den_variable': 0,
    'num_suple': 0,
    'avance_num_suple': 0.0,
    'num_consejeria': 0,
    'avance_num_consejeria': 0.0,
    'num_con_ali': 0,
    'avance_num_con_ali': 0.0,
    'num_con_nutri': 0,
    'avance_num_con_nutri': 0.0,
    'num_dosaje': 0,
    'avance_num_dosaje': 0.0
}
...
DEFAULT_VARIABLES_DETALLADO_DATA = {
    ...
    'd_num_variable': 0,
    'd_num_suple': 0,
    'd_avance_num_suple': 0.0,
    'd_num_consejeria': 0,
    'd_avance_num_consejeria': 0.0,
    'd_num_con_ali': 0,
    'd_avance_num_con_ali': 0.0,
    'd_num_con_nutri': 0,
    'd_avance_num_con_nutri': 0.0,
    'd_num_dosaje': 0,
    'd_avance_num_dosaje': 0.0
}
...
## velocimetro
def obtener_velocimetro_s32_adole_sin_anemia(
    anio: str,
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.callproc('fn_obtener_velocimetro_s32_adole_sin_anemia', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
def obtener_grafico_mensual_s32_adole_sin_anemia(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_grafico_mensualizado_s32_adole_sin_anemia', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
## grafico variables
def obtener_variables_s32_adole_sin_anemia(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_obtener_variables_s32_adole_sin_anemia', [
                anio,
                mes_inicio,
                mes_fin,
                red,
            ...
            if row and len(row) >= 11:
                return [{
                    ...
                    'num_suple': int(row[1]) if row[1] is not None else 0,
                    'avance_num_suple': float(row[2]) if row[2] is not None else 0.0,
                    'num_consejeria': int(row[3]) if row[3] is not None else 0,
                    'avance_num_consejeria': float(row[4]) if row[4] is not None else 0.0,
                    'num_con_ali': int(row[5]) if row[5] is not None else 0,
                    'avance_num_con_ali': float(row[6]) if row[6] is not None else 0.0,
                    'num_con_nutri': int(row[7]) if row[7] is not None else 0,
                    'avance_num_con_nutri': float(row[8]) if row[8] is not None else 0.0,
                    'num_dosaje': int(row[9]) if row[9] is not None else 0,
                    'avance_num_dosaje': float(row[10]) if row[10] is not None else 0.0,
                }]
            else:
                # Sin datos en la tabla o número incorrecto de columnas
                if row:
                    logger.warning(f"La consulta de variables retornó {len(row)} columnas en lugar de 11")
                else:
...
def obtener_variables_detallado_s32_adole_sin_anemia(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_obtener_variables_detallado_s32_adole_sin_anemia', [
                anio,
                mes_inicio,
                mes_fin,
                red,
                ...
                for row in rows:
                    if len(row) >= 21:
                        resultados.append({
                            ...
                            'd_num_suple': int(row[12]) if row[12] is not None else 0,
                            'd_avance_num_suple': float(row[13]) if row[13] is not None else 0.0,
                            'd_num_consejeria': int(row[14]) if row[14] is not None else 0,
...
def obtener_grafico_por_redes_s32_adole_sin_anemia(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_grafico_redes_s32_adole_sin_anemia', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
def obtener_grafico_por_microredes_s32_adole_sin_anemia(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_grafico_microredes_s32_adole_sin_anemia', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
def obtener_grafico_por_establecimientos_s32_adole_sin_anemia(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_grafico_establecimientos_s32_adole_sin_anemia', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
def obtener_seguimiento_s32_adole_sin_anemia(
    anio=None,
    mes_inicio=None,
    mes_fin=None,
    provincia='',
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                """
                SELECT * FROM fn_seg_adole_paquete(
                    %s, %s, %s, %s, %s, %s, %s, %s, %s
                )
                """,
                [

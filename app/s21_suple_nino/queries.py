...
## velocimetro
def obtener_velocimetro_s21_suple_nino(
    anio: str,
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.callproc('fn_obtener_velocimetro_s21_suple_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
def obtener_grafico_mensual_s21_suple_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_grafico_mensualizado_s21_suple_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
## grafico variables
def obtener_variables_s21_suple_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_obtener_variables_s21_suple_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
            ...
            if row and len(row) >= 5:
                return [{
                    ...
                    'num_hierro4m': int(row[1]) if row[1] is not None else 0,
                    'avance_hierro4m': float(row[2]) if row[2] is not None else 0.0,
                    'num_hb6m': int(row[3]) if row[3] is not None else 0,
                    'avance_hb6m': float(row[4]) if row[4] is not None else 0.0
                }]
            else:
                # Sin datos en la tabla o número incorrecto de columnas
                if row:
...
def obtener_variables_detallado_s21_suple_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_obtener_variables_detallado_s21_suple_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
                ...
                for row in rows:
                    if len(row) >= 16:
                        resultados.append({
                            ...
                            'd_num_hierro4m': int(row[12]) if row[12] is not None else 0,
                            'd_avance_hierro4m': float(row[13]) if row[13] is not None else 0.0,
                            'd_num_hb6m': int(row[14]) if row[14] is not None else 0,
                            'd_avance_hb6m': float(row[15]) if row[15] is not None else 0.0
                        })
                    else:
...
def obtener_grafico_por_redes_s21_suple_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_grafico_redes_s21_suple_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
def obtener_grafico_por_microredes_s21_suple_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_grafico_microredes_s21_suple_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
def obtener_grafico_por_establecimientos_s21_suple_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_grafico_establecimientos_s21_suple_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
def obtener_seguimiento_s21_suple_nino(
    anio=None,
    mes_inicio=None,
    mes_fin=None,
    provincia='',

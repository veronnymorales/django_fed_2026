...
DEFAULT_VARIABLES_DATA = {
    'den_variable': 0,
    'num_sindni': 0,
    'avance_num_sindni': 0.0,
    'num_vac_antineumo': 0,
    'avance_num_vac_antineumo': 0.0,
    'num_vac_polio': 0,
    'avance_num_vac_polio': 0.0,
    'num_vac_rota': 0,
    'avance_num_vac_rota': 0.0,
    'num_vac_penta': 0,
    'avance_num_vac_penta': 0.0,
    'num_esq4m': 0,
    'avance_num_esq4m': 0.0,
    'num_esq6m': 0,
    'avance_num_esq6m': 0.0,
    'num_dosaje_hb': 0,
    'avance_num_dosaje_hb': 0.0,
    'num_dniemision30d': 0,
    'avance_num_dniemision30d': 0.0
}
...
DEFAULT_VARIABLES_DETALLADO_DATA = {
    ...
    'd_num_variable':0,
    'd_num_sindni': 0,
    'd_avance_num_sindni': 0.0,
    'd_num_vac_antineumo': 0,
    'd_avance_num_vac_antineumo': 0.0,
    'd_num_vac_polio': 0,
    'd_avance_num_vac_polio': 0.0,
    'd_num_vac_rota': 0,
    'd_avance_num_vac_rota': 0.0,
    'd_num_vac_penta': 0,
    'd_avance_num_vac_penta': 0.0,
    'd_num_esq4m': 0,
    'd_avance_num_esq4m': 0.0,
    'd_num_esq6m': 0,
    'd_avance_num_esq6m': 0.0,
    'd_num_dosaje_hb': 0,
    'd_avance_num_dosaje_hb': 0.0,
    'd_num_dniemision30d': 0,
    'd_avance_num_dniemision30d': 0.0,
}
...
## velocimetro
def obtener_velocimetro_mc21_paquete_nino(
    anio: str,
...
def obtener_grafico_mensual_mc21_paquete_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
...
## grafico variables
def obtener_variables_mc21_paquete_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
        ...
        with connection.cursor() as cursor:
            ...
            if row and len(row) >= 19:
                return [{
                    ...
                    'num_sindni': int(row[1]) if row[1] is not None else 0,
                    'avance_num_sindni': float(row[2]) if row[2] is not None else 0.0,
                    'num_vac_antineumo': int(row[3]) if row[3] is not None else 0,
                    'avance_num_vac_antineumo': float(row[4]) if row[4] is not None else 0.0,
                    'num_vac_polio': int(row[5]) if row[5] is not None else 0,
                    'avance_num_vac_polio': float(row[6]) if row[6] is not None else 0.0,
                    'num_vac_rota': int(row[7]) if row[7] is not None else 0,
                    'avance_num_vac_rota': float(row[8]) if row[8] is not None else 0.0,
                    'num_vac_penta': int(row[9]) if row[9] is not None else 0,
                    'avance_num_vac_penta': float(row[10]) if row[10] is not None else 0.0,
                    'num_esq4m': int(row[11]) if row[11] is not None else 0,
                    'avance_num_esq4m': float(row[12]) if row[12] is not None else 0.0,
                    'num_esq6m': int(row[13]) if row[13] is not None else 0,
                    'avance_num_esq6m': float(row[14]) if row[14] is not None else 0.0,
                    'num_dosaje_hb': int(row[15]) if row[15] is not None else 0,
                    'avance_num_dosaje_hb': float(row[16]) if row[16] is not None else 0.0,
                    'num_dniemision30d': int(row[17]) if row[17] is not None else 0,
                    'avance_num_dniemision30d': float(row[18]) if row[18] is not None else 0.0
                }]
            else:
                # Sin datos en la tabla o número incorrecto de columnas
                if row:
                    logger.warning(f"La consulta de variables retornó {len(row)} columnas en lugar de 19")
                else:
...
def obtener_variables_detallado_mc21_paquete_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
                ...
                for row in rows:
                    if len(row) >= 30:
                        resultados.append({
                            ...
                            'd_num_sindni': int(row[12]) if row[12] is not None else 0,
                            'd_avance_num_sindni': float(row[13]) if row[13] is not None else 0.0,
                            'd_num_vac_antineumo': int(row[14]) if row[14] is not None else 0,
                            'd_avance_num_vac_antineumo': float(row[15]) if row[15] is not None else 0.0,
                            'd_num_vac_polio': int(row[16]) if row[16] is not None else 0,
                            'd_avance_num_vac_polio': float(row[17]) if row[17] is not None else 0.0,
                            'd_num_vac_rota': int(row[18]) if row[18] is not None else 0,
                            'd_avance_num_vac_rota': float(row[19]) if row[19] is not None else 0.0,
                            'd_num_vac_penta': int(row[20]) if row[20] is not None else 0,
                            'd_avance_num_vac_penta': float(row[21]) if row[21] is not None else 0.0,
                            'd_num_esq4m': int(row[22]) if row[22] is not None else 0,
                            'd_avance_num_esq4m': float(row[23]) if row[23] is not None else 0.0,
                            'd_num_esq6m': int(row[24]) if row[24] is not None else 0,
                            'd_avance_num_esq6m': float(row[25]) if row[25] is not None else 0.0,
                            'd_num_dosaje_hb': int(row[26]) if row[26] is not None else 0,
                            'd_avance_num_dosaje_hb': float(row[27]) if row[27] is not None else 0.0,
                            'd_num_dniemision30d': int(row[28]) if row[28] is not None else 0,
                            'd_avance_num_dniemision30d': float(row[29]) if row[29] is not None else 0.0
                        })
                    else:
                        logger.warning(f"Una fila retornó {len(row)} columnas en lugar de 30, omitiendo...")
...
def obtener_grafico_por_redes_mc21_paquete_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
...
def obtener_grafico_por_microredes_mc21_paquete_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
...
def obtener_grafico_por_establecimientos_mc21_paquete_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
...
def obtener_seguimiento_mc21_paquete_nino(
    anio=None,
    mes_inicio=None,
    mes_fin=None,
    provincia='',
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                """
                SELECT * FROM fn_seg_mc21_paquete_nino(
                    %s, %s, %s, %s, %s, %s, %s, %s, %s
                )
                """,
                [

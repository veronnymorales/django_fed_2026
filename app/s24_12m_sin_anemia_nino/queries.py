...
DEFAULT_VARIABLES_DATA = {
    'den_variable': 0,
    'num_suple_inicio': 0,
    'avance_suple_inicio': 0.0,
    'num_suple_continuacion': 0,
    'avance_suple_continuacion': 0.0,
    'num_hb9m': 0,
    'avance_hb9m': 0.0,
    'num_hb12m': 0,
    'avance_hb12m': 0.0
...
DEFAULT_VARIABLES_DETALLADO_DATA = {
    ...
    'd_den_variable': 0,
    'd_num_variable': 0,
    'd_num_suple_inicio':0,
    'd_avance_suple_inicio':0.0,
    'd_num_suple_continuacion':0,
    'd_avance_suple_continuacion':0.0,
    'd_num_hb9m':0,
    'd_avance_hb9m':0.0,
    'd_num_hb12m':0,
    'd_avance_hb12m':0.0
...
## velocimetro
def obtener_velocimetro_s24_12m_sin_anemia_nino(
    anio: str,
    ...
    try:
        with connection.cursor() as cursor:
            ...
            cursor.callproc('fn_obtener_velocimetro_s24_12m_sin_anemia_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
def obtener_grafico_mensual_s24_12m_sin_anemia_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_grafico_mensualizado_s24_12m_sin_anemia_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
## grafico variables
def obtener_variables_s24_12m_sin_anemia_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_obtener_variables_s24_12m_sin_anemia_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
            ...
            if row and len(row) >= 9:
                return [{
                    ...
                    'num_suple_inicio': int(row[1]) if row[1] is not None else 0,
                    'avance_suple_inicio': float(row[2]) if row[2] is not None else 0.0,
                    'num_suple_continuacion': int(row[3]) if row[3] is not None else 0,
                    'avance_suple_continuacion': float(row[4]) if row[4] is not None else 0.0,
                    'num_hb9m': int(row[5]) if row[5] is not None else 0,
                    'avance_hb9m': float(row[6]) if row[6] is not None else 0.0,
                    'num_hb12m': int(row[7]) if row[7] is not None else 0,
                    'avance_hb12m': float(row[8]) if row[8] is not None else 0.0
                }]
            else:
                # Sin datos en la tabla o número incorrecto de columnas
                if row:
                    logger.warning(f"La consulta de variables retornó {len(row)} columnas en lugar de 9")
                else:
...
def obtener_variables_detallado_s24_12m_sin_anemia_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_obtener_variables_detallado_s24_12m_sin_anemia_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
                ...
                for row in rows:
                    if len(row) >= 20:
                        resultados.append({
                            ...
                            'd_num_suple_inicio': int(row[12]) if row[12] is not None else 0,
                            'd_avance_suple_inicio': float(row[13]) if row[13] is not None else 0.0,
                            'd_num_suple_continuacion': int(row[14]) if row[14] is not None else 0,
                            'd_avance_suple_continuacion': float(row[15]) if row[15] is not None else 0.0,
                            'd_num_hb9m': int(row[16]) if row[16] is not None else 0,
                            'd_avance_hb9m': float(row[17]) if row[17] is not None else 0.0,
                            'd_num_hb12m': int(row[18]) if row[18] is not None else 0,
                            'd_avance_hb12m': float(row[19]) if row[19] is not None else 0.0
                        })
                    else:
                        logger.warning(f"Una fila retornó {len(row)} columnas en lugar de 20, omitiendo...")
...
def obtener_grafico_por_redes_s24_12m_sin_anemia_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_grafico_redes_s24_12m_sin_anemia_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
def obtener_grafico_por_microredes_s24_12m_sin_anemia_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_grafico_microredes_s24_12m_sin_anemia_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
def obtener_grafico_por_establecimientos_s24_12m_sin_anemia_nino(
    anio: str,
    mes_inicio: Optional[str],
    mes_fin: Optional[str],
    red: Optional[str],
    ...
    try:
        with connection.cursor() as cursor:
            # Llamar a la función almacenada con los parámetros en orden
            cursor.callproc('fn_grafico_establecimientos_s24_12m_sin_anemia_nino', [
                anio,
                mes_inicio,
                mes_fin,
                red,
...
def obtener_seguimiento_s24_12m_sin_anemia_nino(
    anio=None,
    mes_inicio=None,
    mes_fin=None,
    provincia='',
    ...
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                """
                SELECT * FROM fn_seg_12m_sin_anemia_nino(
                    %s, %s, %s, %s, %s, %s, %s, %s, %s
                )
                """,
                [
